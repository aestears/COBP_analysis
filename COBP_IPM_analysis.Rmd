...
{
  linear.p <- m.par["flow.int"] + m.par["flow.z"] * z  + m.par["flow.z.sq"]*z^2     # linear predictor
  p <- 1/(1+exp(-linear.p))                                # logistic transformation to probability
  return(p)
}
```
...
Look at ipmr functions to figure out what to do next
```{r}
library(ipmr)
...
### Define the growth/survival kernel
```{r}
...
### Define the function for new recruits from the seedling stage
```{r}
#??? but this isn't an integral? what to do about this??
NewRosette_fxn <-  function (z1, s, m.par) {
  #z1 size of new rosette in year t+1
  #s size of the seedbank at size t
...
F_z1z <- function (z1, z, m.par) {
  ...
  return( p_bz(z, m.par) * b_z(z, m.par) * m.par["p.estab"] * c_0z1(z1, m.par))
...
get_mean_var_Repr <- function(init.z,n.samp) {
  ...
  repeat {
    ...
    if(num.Repr>0) {
      Seeds <- rpois(num.Repr, m.par.true["p.estab"] * b_z(z[Repr==1],m.par.true))
...
names(m.par.est) <- c("surv.int", "surv.z", "flow.int", "flow.z", "flow.z.sq" , "grow.int", "grow.z",   "grow.sd" , "rcsz.int", "rcsz.sd" , "seed.int", "seed.z"  , "p.estab", "delta", "p.r", "gamma", "s.b")  
...
image(IPM.est$meshpts,IPM.est$meshpts, t(IPM.est$F)^.1)
```
