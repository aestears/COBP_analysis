...
NOT A PROBABILITY-- should use log, rather that logit transformation???
```{r}
# GROWTH (we assume a constant variance)
GR.fun <- function(z, zz, paramCont) {
  growth.mu =
...
growth <-
  lapply(allIPMs,
    FUN = function(x)
      t(outer(meshp, meshp, GR.fun, paramCont = x$params) ## don't multiply by h (integrate)...should I???
            )
    ) 
...
corr.growth.sd <-
  apply(log(simplify2array(growth)), 1:2, sd)
```
...
Elasticity for $G(z',z)$ = $$\frac{\delta \lambda}{\delta G(z',z)} * \frac{G(z',z)}{\lambda} $$
...
??? Can we do this across the entire set of vital rates ???
```{r}
bigDF <- data.frame("VSS" = c(VSS.surv, VSS.seeds, VSS.flwr, as.vector(VSS.growth)
                              ),
                    "corrSD" = c(corr.surv.sd, corr.seedProd.sd, corr.flowering.sd#, as.vector(corr.growth.sd)
                                 )
                                 )
...
bigDF_noOut <- bigDF[bigDF$VSS != max(bigDF$VSS),]
bigDF_noOut_cor <- cor(bigDF_noOut$VSS, bigDF_noOut$corrSD)

plot(x = bigDF_noOut$VSS, y = bigDF_noOut$corrSD)
text(x = 30, y = 2, labels = paste0("Corr = ", round(bigDF_noOut_cor, 4)))
```
