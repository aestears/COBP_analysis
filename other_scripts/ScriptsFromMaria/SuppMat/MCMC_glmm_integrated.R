...
# the variance stabilized integrated sensitivity of the populaiton growth rate as a function of standardized vital-rate variation,
...
vss.data <- read.csv("vss.data.integr.csv")

colnames(vss.data)[4]="vss"
...
vss.data$pred_m1_fit=predict(m1,interval="confidence",marginal=NULL)[,1]
vss.data$pred_m1_lrw=predict(m1,interval="confidence",marginal=NULL)[,2]
vss.data$pred_m1_upr=predict(m1,interval="confidence",marginal=NULL)[,3]
...
vss.data$pred_m2_fit=predict(m2,interval="confidence",marginal=NULL)[,1]
vss.data$pred_m2_lrw=predict(m2,interval="confidence",marginal=NULL)[,2]
vss.data$pred_m2_upr=predict(m2,interval="confidence",marginal=NULL)[,3]
...
vss.data$pred_m3_fit=predict(m3,interval="confidence",marginal=NULL)[,1]
vss.data$pred_m3_lrw=predict(m3,interval="confidence",marginal=NULL)[,2]
vss.data$pred_m3_upr=predict(m3,interval="confidence",marginal=NULL)[,3]


ggplot(vss.data,aes(sd.corr,vss))+
  geom_point()+
...
ggplot(df,aes(vss_prop_phlyo_mean))+
  ...
  geom_density(aes(vss_prop_phlyo_slope),fill="orange",alpha=0.5)+
