...
Calculate the un-corrected sensitivity of the IPM matrix to changes in the survival function by calculating the IPM matrix with slightly purturbed survival function 
```{r}
# Setting up the kernels
K <- array(0,c(n+1,n+1))
b <- L+c(0:n)*(U-L)/n # interval that each cell of the matrix covers 
...
S.fun_pert <- function(z, paramCont) {
  mu.surv=paramCont[["survival"]]["(Intercept)",] + paramCont[["survival"]]["log_LL_t",]*z - perturbVal
  return(1/(1 + exp(-(mu.surv))))
}
# Survival and growth 
...
```{r}
# sens_Surv_1 <- as.numeric(lambda_perturb_Survival - lambda_OG) / (s_perturb - surv_OG)
#sens_Surv_2 <- as.numeric(lambda_perturb_Survival - lambda_OG) / (s_perturb - surv_OG)
#sens_Surv_3 <- as.numeric(lambda_perturb_Survival - lambda_OG) / (s_perturb - surv_OG)
sens_Surv_ <- as.numeric(lambda_perturb_Survival - lambda_OG) / (s_perturb - surv_OG)
