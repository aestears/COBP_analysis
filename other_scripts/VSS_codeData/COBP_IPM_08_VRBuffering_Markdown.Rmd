...
%%% I'm really not sure where I should add the perturbation to this function? Should it be before it goes through the logit transformation??
```{r}
# Setting up the kernels
K <- array(0,c(n+1,n+1))
b <- L+c(0:n)*(U-L)/n # interval that each cell of the matrix covers 
...
SDP.fun_pert <- function(z, paramCont) {
  mu.fps=exp(paramCont[["seedProduction"]][1,1] + paramCont[["seedProduction"]][2,1]*z) + perturbVal 
  return(mu.fps)
}
...
plot(VSS.seeds, type = "l", ylim = c(0, 4.5))
lines(sens_SeedProd, lty = 2)
```
