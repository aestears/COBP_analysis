...
#### We don't calculate mean for seedbank parameters and seedling size distribution, because those values were calculated at the population level (so there is no variation accross subpopulations). We simulate standard deviation for these vital rates 
...
# calculate VSS 
# instead of mean seedling size, just use seedlingSize_OG, since they're all simulated...
VSS.SeedlingSize <- as.numeric(sens_SeedlingSize * ((seedlingSize_OG*(1-seedlingSize_OG))/lambda_OG))
...
bigDF <- data.frame("VitalRate" = c(rep("surv", 500), rep("seedProd", 500), rep("flowerProb", 500), rep("growth", 500)),
                    "sizeClass" = rep(1:500, times = 4),
                    "VSS" = c(VSS.surv, VSS.seeds, VSS.flwr, VSS.growth), 
                    "correctedSD" = c(corr.surv.sd, corr.seedProd.sd, corr.flowering.sd, corr.growth.sd)
                    )

for (i in 1:500) {
  # calculate the correlation
  cor_temp <- cor.test(bigDF[bigDF$sizeClass == i, "VSS"], bigDF[bigDF$sizeClass == i, "correctedSD"], method = "pearson")
...
# visualize the correlation coefficiennts for each size class 
...
# correlation between VSS and corrected standard deviation
bigDF_cor <- cor(bigDF$VSS, bigDF$corrSD)

plot(x = bigDF$VSS, y = bigDF$corrSD)
text(x = 2, y = 2, labels = paste0("Corr = ", round(bigDF_cor, 4)))
