...
## calculate neighbors w/in a 20 cm radius
dat_4 <- plantTracker::getNeighbors(dat = dat_3, buff = 20, method = "count", compType = "oneSpp", quad = "Plot_ID")
## rename column to indicate radius
names(dat_4)[6] <-"neighbors_20"

## calculate neighbors w/in a 40 cm radius
dat_5 <- plantTracker::getNeighbors(dat = dat_4, buff = 40, method = "count", compType = "oneSpp", quad = "Plot_ID")
## rename column to indicate radius
names(dat_5)[6] <-"neighbors_40"

## calculate neighbors w/in a 60 cm radius
dat_6 <- plantTracker::getNeighbors(dat = dat_5, buff = 60, method = "count", compType = "oneSpp", quad = "Plot_ID")
## rename column to indicate radius
names(dat_6)[6] <-"neighbors_60"

## calculate neighbors w/in a 100 cm radius
dat_7 <- plantTracker::getNeighbors(dat = dat_6, buff = 100, method = "count", compType = "oneSpp", quad = "Plot_ID")
## rename column to indicate radius
names(dat_7)[6] <-"neighbors_100"
...
```{r}
## growth models for each radius
neighMod_5 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_5, data = dat)
summary(neighMod_5)
neighMod_10 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_10, data = dat)
summary(neighMod_10)
neighMod_15 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_15, data = dat)
summary(neighMod_15)
neighMod_20 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_20, data = dat)
summary(neighMod_20)
neighMod_40 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_40, data = dat)
summary(neighMod_40)
neighMod_60 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_60, data = dat)
summary(neighMod_60)
neighMod_100 <- lm(log_LL_tplus1 ~ log_LL_t + neighbors_100, data = dat)
summary(neighMod_100)
```

Then, visually compare models
```{r}
plot(log_LL_tplus1 ~ log_LL_t, data = dat)
abline(lm(log_LL_tplus1 ~ log_LL_t, data = dat), col = "red") # add model w/out neighbor term
#make data for predictions
newdata <- data.frame("log_LL_t" = seq(from = min(dat$log_LL_t, na.rm = TRUE), 
               to = max(dat$log_LL_t, na.rm = TRUE),
               length.out = 100), "neighbors_5" = mean(dat$neighbors_5), "neighbors_10" = mean(dat$neighbors_10), "neighbors_15" = mean(dat$neighbors_15), "neighbors_20" = mean(dat$neighbors_20), "neighbors_40" = mean(dat$neighbors_40))
lines(x = newdata$log_LL_t, y = predict(object = neighMod_5, newdata =  newdata), col = "blue")
lines(x = newdata$log_LL_t, y = predict(object = neighMod_10, newdata =  newdata), col = "green")
lines(x = newdata$log_LL_t, y = predict(object = neighMod_15, newdata =  newdata), col = "orange")
lines(x = newdata$log_LL_t, y = predict(object = neighMod_20, newdata =  newdata), col = "pink")
lines(x = newdata$log_LL_t, y = predict(object = neighMod_40, newdata =  newdata), col = "purple")
#abline(a = coef(neighMod_5)[1], b = coef(neighMod_5)[2], col = "blue")
#abline(a = coef(neighMod_10)[1], b = coef(neighMod_10)[2], col = "green")
#abline(a = coef(neighMod_15)[1], b = coef(neighMod_15)[2], col = "orange")
abline(a = 0, b = 1, col = "darkgrey", lty = 2) ## add a 1:1 line
```

Now, compare the models using AIC
```{r}
AIC(lm(log_LL_tplus1 ~ log_LL_t, data = dat), neighMod_5, neighMod_10, neighMod_15, neighMod_20, neighMod_40, neighMod_60, neighMod_100)

## plot the decline in AIC with radius
plot(x =c(0,5,10,15,20,40,60,100), y = c(3186.305,3115.361,3046.081,3019.100, 3002.569, 2946.028, 2935.554, 2924.782), type = "l")

## plot the change in effect size w/ size of radius
plot(x =c(0,5,10,15,20,40,60,100), y = c(NA, coef(neighMod_5)[3], coef(neighMod_10)[3], coef(neighMod_15)[3], coef(neighMod_20)[3], coef(neighMod_40)[3], coef(neighMod_60)[3], coef(neighMod_100)[3]), type = "l")
```

Maybe use neighbors_20? (since the AIC decreases, but the effect size is smaller)
...
newdata <- data.frame("log_LL_t" = seq(from = min(survDat$log_LL_t, na.rm = TRUE), 
               ...
               length.out = 100), "neighbors_20" = mean(survDat$neighbors_20))
...
newdata <- data.frame("log_LL_t" = seq(from = min(dat$log_LL_t, na.rm = TRUE), 
               to = max(dat$log_LL_t, na.rm = TRUE),
               length.out = 100), "neighbors_20" = mean(dat$neighbors_20))
...
newdata <- data.frame("log_LL_t" = seq(from = min(seedDat$log_LL_t, na.rm = TRUE), 
                                       ...
                                       length.out = 100), "neighbors_20" = mean(seedDat$neighbors_20))
...
graphics::layout(mat = matrix(c(1,4, 7, 2, 5, 8, 3, 6, 9),nrow = 3, ncol = 3), widths = c(1,1,5),heights = c(5,1,1))
par(mar = c(1,3,3,1))
image(t(mega_mat$mega_matrix[1,3:502]^.1), xaxt = "n", 
      col = pal[(round(min( t(mega_mat$mega_matrix[1,3:502])^.1),2)*100): 
                  (round(max( t(mega_mat$mega_matrix[1,3:502])^.1),2)*100)]) ## B(t+1)
par(mar = c(1,1,3,1))
image(t(mega_mat$mega_matrix[2,3:502]^.1), xaxt = "n", 
      col = pal[(round(min( t(mega_mat$mega_matrix[2,3:502])^.1),2)*100): 
                  (round(max( t(mega_mat$mega_matrix[2,3:502])^.1),2)*100)]) ## S(t+1)
par(mar = c(1,1,3,3))
image(x = meshpts, y = meshpts, t(mega_mat$mega_matrix[3:502,3:502])^.1,
      xlab = "n(z,t)", ylab = "n(z',t+1)", main = "K matrix",
...
par(mar = c(1,3,1,1))
image(as.matrix(0), xaxt = "n", yaxt = "n", col = "white") # seedlings to seedbank
par(mar = c(1,1,1,1))
image(as.matrix(0), xaxt = "n", yaxt = "n",  col = "white") # seedlings to seedlings
par(mar = c(1,1,1,3))
image(t(mega_mat$mega_matrix[3:502,1]^.1), yaxt = "n", 
        col = pal[(round(min( t(mega_mat$mega_matrix[3:502,2])^.1),2)*100): 
                    (round(max( t(mega_mat$mega_matrix[3:502,2])^.1),2)*100)]) #S(t)
par(mar = c(3,3,1,1))
...
image(t(mega_mat$mega_matrix[3:502,1]^.1), yaxt = "n", 
        col = pal[(round(min( t(mega_mat$mega_matrix[3:502,1])^.1),2)*100): 
                    (round(max( t(mega_mat$mega_matrix[3:502,1])^.1),2)*100)]) ## B(t)(is all zeros--can't transition to continuous stage from the seedbank)
```
...
## add the seedling vector
  hist(make_iter_kernel(ipm = general_ipm, mega_mat = c(stay_seedbank,repro_to_seedbank))$mega_matrix)
## add the seedbank vector
