...
### Probability that a seed from the seedbank in year t will stay in teh seedbank in year t+1
$$staySB$$
```{r}
staySB.est <- .4
```
...
## make sure data is in the correct format for IPMpack
datP <- dat
datP$size <- datP$log_LL_t
datP$sizeNext <- datP$log_LL_tplus1
datP$surv <- datP$survives_tplus1
datP$fec1 <- datP$flowering # probability of reproducing
datP$fec2 <- datP$Num_capsules # number of seeds produced 
datP$stage <- "continuous" 
datP$stageNext <- NA #specify the stage in the next year as "continuous"
  datP[datP$surv==0 & is.na(datP$surv) == FALSE, "stageNext"] <- "dead" # next stage for dead individuals is "dead"
  datP[datP$surv==1 & is.na(datP$surv) == FALSE, "stageNext"] <- "continuous"
datP$number <- 1 
# add data for seedbank stage
  # seeds that survived to germinate (exited the seedbank)
  # seeds that did not germinate
# add data for seedling stage
  # seedlings that 
